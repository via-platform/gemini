...
class GeminiAdapter {
    ...
    history(symbol){
        const id = symbol.name.split('-').join('').toLowerCase();

        return axios.get(`${base}/trades/${id}`, {params: {limit_trades: 100}})
        .then(response => response.data.map(datum => {
            return {
                date: new Date(datum.timestampms),
                id: datum.tid,
                price: parseFloat(datum.price),
                size: parseFloat(datum.amount),
                side: (datum.type === 'sell') ? 'sell' : 'buy'
            };
        }));
    }
